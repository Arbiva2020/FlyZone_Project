...
DATABASE_URL = "sqlite:///C:/Users/ADI/Desktop/flyzoneproj/flyzone_server/flyzone.db"
...
class User(Base):
    ...
    number_of_failures = Column(Integer, nullable=True)
    straight_failures = Column(Integer, nullable=True)
    ...
    levelResults = relationship("LevelResults", back_populates="user")
...
class LevelResults(Base):
    __tablename__ = 'levelResults'
    id = Column(Integer, primary_key=True, index=True)
    level_level = Column(String, index=True)
    user_id = Column(Integer, ForeignKey('users.id'))
    user = relationship("User", back_populates="levelResults")
    fog_level = Column(Integer)
    brightness_level = Column(Integer)
    wind_level = Column(Integer)
    close_calls = Column(Integer)
